...
function installAltadore(){
 ...
 let n=0
 const show=()=>{hero.style.backgroundImage=`linear-gradient(rgba(16,48,39,.50),rgba(16,48,39,.76)),url("${ALTADORE_IMAGES[n]}")`;n=(n+1)%ALTADORE_IMAGES.length}
 ...
 section.innerHTML=`<div class="wrap"><div class="altadore-copy"><div class="eyebrow">A place in our family story</div><h2>Altadore Estate, County Wicklow</h2><p>Altadore is an important place in the Medcalf family story. Enoch Medcalf was born at Altadore in 1874, and his 1895 marriage record identifies his father, Anthony Metcalf, as a gardener. These photographs bring the house, gardens and landscape connected with that generation into the family archive.</p></div><div class="altadore-gallery">${ALTADORE_IMAGES.map((src,i)=>`<figure class="altadore-photo"><img src="${src}" alt="${['Altadore house and garden','Altadore garden','Altadore estate water garden','Altadore estate and gardens'][i]}" loading="lazy"><span>${['House & garden','Garden','Estate water garden','Estate grounds'][i]}</span></figure>`).join('')}</div><p class="altadore-source">Historical location imagery: <a href="${sourceUrl}" target="_blank" rel="noopener">Irish Historic Houses — Altidore Castle</a></p></div>`
