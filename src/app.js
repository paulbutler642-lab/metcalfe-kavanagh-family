import { evidenceLabels, familyHistoryFor, verifiedRecordsFor } from '/family-history-archive.js?v=20260914-william-boxing-1'
...
async function home() {
  ...
  app.innerHTML = `<section class="hero"><div class="wrap"><h1>The Metcalfe & Kavanagh Family</h1><p>Our Family History • Our Stories • Our Heritage</p><a class="btn" href="/?view=tree">Explore Our Family Tree</a></div></section><section class="section"><div class="wrap"><div class="homegrid"><a class="feature" href="/?view=tree"><span class="ico">🌳</span><strong>Family Tree</strong><small>Explore generations and relationships</small></a><a class="feature" href="/?view=people"><span class="ico">👥</span><strong>People</strong><small>Browse family profiles</small></a><a class="feature" href="/?view=visitors"><span class="ico">✒️</span><strong>Visitors’ Book</strong><small>Sign your name or share a family connection</small></a><a class="feature" href="/?view=gallery"><span class="ico">🗂️</span><strong>Photos &amp; Documents</strong><small>Explore the family archive</small></a></div><div class="subgrid"><a class="feature" href="/?view=sources"><span class="ico">📜</span><strong>Research &amp; Sources</strong><small>Explore census returns and historical records</small></a><a class="feature" href="/?view=places"><span class="ico">📍</span><strong>Places</strong><small>Locations connected to the family</small></a><a class="feature" href="/?view=timeline"><span class="ico">🕰️</span><strong>Timeline</strong><small>The family story in date order</small></a><a class="feature" href="/?view=stories"><span class="ico">✦</span><strong>Stories</strong><small>Lives, memories and family history</small></a></div></div></section><section class="section" style="background:#eef3ee"><div class="wrap"><h2>Featured Ancestors</h2><div class="person-grid">${featured.map(personCard).join('')}</div></div></section>`
}
...
async function profile() {
  ...
  if (parents.length) bits.push(`${parents.length > 1 ? 'Their recorded parents are' : 'Their recorded parent is'} ${parents.map((x) => x.name).join(' and ')}.`)
  if (spouses.length) bits.push(`${p.name} is recorded as the spouse of ${spouses.map((x) => x.name).join(' and ')}.`)
  if (children.length) bits.push(`Recorded children include ${children.map((x) => x.name).join(', ')}.`)
