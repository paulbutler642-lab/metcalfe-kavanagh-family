...
async function render(){
  ...
  const [{data:people,error:pe},{data:media,error:me}]=await Promise.all([
  ...
  const card=p=>{const m=profileFor(p.id);return `<a class="card person people-card" href="/?view=profile&id=${encodeURIComponent(p.id)}">${m?`<img class="people-avatar-img" src="${mediaUrl(m)}" alt="${esc(p.name)}">`:`<div class="avatar">${esc(initials(p))}</div>`}<div class="people-card-copy"><strong>${esc(p.name)}</strong><div class="muted">${esc(years(p)||p.relation_label||'Family member')}</div>${p.birth_place?`<small>${esc(p.birth_place)}</small>`:''}${p.biography?`<p class="people-snippet">${esc(p.biography)}</p>`:''}</div></a>`};

  app.innerHTML=header('People','Browse the Metcalfe / Medcalf and Kavanagh sides of the family separately.')+`<section class="section people-section"><div class="wrap">
    <style>
  ...
  const grid=document.getElementById('peopleGrid'),title=document.getElementById('peopleLineTitle'),count=document.getElementById('peopleCount');
  const paint=()=>{const list=groups[active];title.textContent=active==='metcalfe'?'Metcalfe / Medcalf Family':'Kavanagh Family';count.textContent=`${list.length} ${list.length===1?'person':'people'}`;grid.innerHTML=list.map(card).join('')||'<div class="card">No people have been added to this family line yet.</div>';};
  document.querySelectorAll('.people-tab').forEach(btn=>btn.addEventListener('click',()=>{active=btn.dataset.line;document.querySelectorAll('.people-tab').forEach(b=>{const on=b===btn;b.classList.toggle('active',on);b.setAttribute('aria-selected',String(on))});paint();}));
  paint();
}

render().catch(e=>{console.error(e);app.innerHTML=header('People','The people section could not load.')+`<section class="section"><div class="wrap"><div class="status error">${esc(e.message||e)}</div></div></section>`});
