...
function relevant(question,people,sources,links,parentChild,couples,relationships){
  ...
  const exactNamed=people.filter(p=>{const tokens=nameTokens(p);return tokens.length&&tokens.some(t=>t.length>2&&words.includes(t))}).map(p=>({p,hits:nameTokens(p).filter(t=>words.includes(t)).length})).sort((a,b)=>b.hits-a.hits);
...
function gatewayDiagnostic(status){if(status===400)return 'gateway-400-request';if(status===401)return 'gateway-401-auth';if(status===402)return 'gateway-402-credit';if(status===403)return 'gateway-403-access';if(status===404)return 'gateway-404-model';if(status===429)return 'gateway-429-limit';return `gateway-${status||'error'}`}
function runtimeToken(req){const h=req.headers?.['x-vercel-oidc-token'];return clean(Array.isArray(h)?h[0]:h)||clean(process.env.VERCEL_OIDC_TOKEN)||clean(process.env.AI_GATEWAY_API_KEY)}
export default async function handler(req,res){
  ...
  try{
    ...
    const evidence=relevant(question,people,sources,links,parentChild,couples,relationships),recordCount=evidence.people.length+evidence.sources.length,gatewayToken=runtimeToken(req);let out;
    if(gatewayToken){try{
      ...
      const completion=await ai.json(),text=completion.choices?.[0]?.message?.content||'',raw=text.replace(/^```json\s*|\s*```$/g,'').trim();out=JSON.parse(raw);
    }catch(aiError){console.error('family-historian-ai',aiError);out=fallback(evidence,gatewayDiagnostic(aiError.status))}}
    else{console.error('family-historian-ai','No Vercel OIDC token was available at runtime');out=fallback(evidence,'oidc-token-missing')}
